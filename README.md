...
Validation is implemented as the locked PEP 723 script `scripts/validate.py`. It stages wrapper charts under `.validation/`, builds their committed dependency locks without changing source charts, checks rendered resource contracts, and runs native Mimir, Tempo, Loki, Alloy, and Envoy configuration validators where applicable. Successful runs remove their workspace; failed runs preserve it for diagnosis.
