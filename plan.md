...
- Added the desired-state Alloy trace pipeline with dedicated batching, bounded non-blocking retries, and native mTLS export through the authenticated Tempo gateway. Application trace export remains disabled until the collector rollout is live-verified.
...
- [x] Install and live-verify Tempo with dedicated least-privilege access to the existing MinIO `tempo` bucket and explicit tenant `k8s-playground`.
