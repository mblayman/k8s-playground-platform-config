...
- Added and live-verified the desired-state Alloy trace pipeline with dedicated batching, bounded non-blocking retries, native mTLS export through the authenticated Tempo gateway, exact application identity authorization, and single-span delivery without cross-tenant visibility. Application trace export is enabled in desired state pending rollout and live verification.
